...
⚠️  ATSE-3 FIX (v1.2.0): Bu dosya artık engine/ats_engine/scoring.py üzerinden
çalışan ince bir sarmalayıcıdır (thin wrapper). Çift motor sürüklenmesi giderildi.

Eski kullanım hâlâ çalışır:
    from ats_score import ats_match_score
    python ats_score.py --jd jd.txt --cv cv.txt --must "akreditif,incoterms"
...
# Engine modülünü import yoluna ekle
_ENGINE_ROOT = os.path.normpath(
...
try:
    from ats_engine.scoring import (  # noqa: F401
        ats_match_score,
        tfidf_cosine,
        coverage,
        stuffing_penalty,
    ...
    from ats_engine.text import tokenize  # noqa: F401
    from ats_engine.bm25 import BM25  # noqa: F401

    _ENGINE_AVAILABLE = True
except ImportError:
    _ENGINE_AVAILABLE = False
    warnings.warn(
        "ats_engine modülü bulunamadı — bağımsız (standalone) mod kullanılıyor. "
        "Bu mod artık korunmuyor; lütfen engine/ dizininden çalıştırın.",
        DeprecationWarning,
        stacklevel=2,
    )

    # ── Bağımsız yedek (legacy) — yalnızca engine import edilemezse ──
    import math
    import re
    ...
    STOP = set(
        "ve veya ile için bir bu şu da de ki mi mu çok daha en gibi olarak "
        "the a an and or for to of in on with as is are be by at from that this".split()
    )
    ...
    def prf(cv_tokens, must_have):
        cv_set = set(cv_tokens)
        cv_text = " ".join(cv_tokens)
        M = set(m.lower().strip() for m in must_have if m.strip())
        if not M:
            return 0.0, 0.0, 0.0
    ...
    def ats_match_score(jd_text, cv_text, must_have, corpus_texts=None,
        ...
        warnings.warn(
            "Bağımsız ats_score.py artık korunmuyor. "
            "ats_engine.scoring.ats_match_score kullanın.",
            DeprecationWarning,
            stacklevel=2,
        )
        ...
        return {
            ...
            "components": {
                "Lex": round(Lex, 3), "Sem": (round(Sem, 3) if Sem is not None else "yok"),
                "Cov": round(Cov, 3), "Parse_gate": parse_gate, "Stuffing": round(Stuff, 3),
            },
...
def main():
    ...
    if args.corpus and os.path.isdir(args.corpus):
        corpus = [
            _read(os.path.join(args.corpus, f))
